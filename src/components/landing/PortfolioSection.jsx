...
export default function PortfolioSection() {
  return (
    <section id="portfolio" className="py-20" aria-labelledby="portfolio-title">
      <Reveal className="mx-auto mb-12 max-w-7xl px-6 lg:px-10">
      ...
      <div className="grid gap-6 px-4 md:grid-cols-12 md:px-6">
        {projects.map((project, i) =>
          <Reveal key={project.slug} delay={i * 0.15} className={`${project.wide ? 'md:col-span-7' : 'md:col-span-5'}`}>
            <Link to={`/project/${project.slug}`} className="group relative block h-full overflow-hidden rounded-[2rem]">
