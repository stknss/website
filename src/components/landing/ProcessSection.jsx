    ...
    <section id="process" className="px-6 py-24 lg:px-10 lg:py-32" aria-labelledby="process-title">
      <div className="mx-auto max-w-7xl">
        <div className="mb-16 grid gap-6 lg:grid-cols-12">
          ...
          <h2 id="process-title" className="font-display text-5xl font-light italic leading-none md:text-7xl lg:col-span-7">
            Тёплая идея проходит через строгую инженерную систему
          </h2>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {steps.map(([number, title, text]) => (
            <article key={number} className="group min-h-72 border border-border bg-card/40 p-7 transition duration-300 hover:-translate-y-2 hover:bg-secondary/70">
              <span className="font-mono text-sm text-primary">{number}</span>
              <h3 className="mt-16 text-2xl font-medium text-foreground">{title}</h3>
